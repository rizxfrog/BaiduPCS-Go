...
```
  ...
  --check         强制在下载完成后校验文件大小和可用的完整文件 MD5
...
比较网盘文件与已下载本地文件的大小和完整文件 MD5：

```
BaiduPCS-Go verify <网盘文件> <本地文件>
```

网盘文件可使用路径、`$<FS ID>` 或 `%<MD5>`。在系统 Shell 中使用 `$` 时需要加引号，例如：
...
只有网盘返回可用的单文件 MD5 时才能完成 MD5 校验；多分片 MD5、目录或缺少完整 MD5 的文件会明确报告不支持。`download --check` 可忽略全局 `no_check` 配置，为本次下载强制开启校验。
...
* 开启MD5校验下载时可能有 check MD5 不通过, 但文件其实并未出错的情况, 使用 `--nocheck` 下载或配置中启用 `no_check` 即可(3.7版本默认已启用).
